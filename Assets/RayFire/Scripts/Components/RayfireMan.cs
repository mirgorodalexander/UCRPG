...
namespace RayFire
{
    ...
    public class RayfireMan : MonoBehaviour
    {
        ...
        [Header("  Infinite Fall")]
        [Space(2)]
        
        public bool destroy = false;
        [Range(1f, 300f)] public float distance = 30f;
        ...
        public static int buildMinor = 14;
        ...
        public static void DestroyFragment(GameObject go, Transform tm, float time = 0f)
        {
            ...
            // Parent to man
            if (inst.advancedDemolitionProperties.demolishedObject == RFManDemolition.PostDemolitionType.Deactivate)
                go.transform.parent = inst.transForm;
            
            // Destroy
            if (inst.advancedDemolitionProperties.demolishedObject == RFManDemolition.PostDemolitionType.Destroy)
                DestroyOp(go, tm, time);
        }
        
        // Check if fragment is the last child in root and delete root as well
        public static void DestroyOp(GameObject go, Transform tm, float time = 0f)
        {
            // Set delay
            if (time == 0)
                time = inst.advancedDemolitionProperties.destroyDelay;
            
            // Destroy with root if last
            if (tm != null && tm.childCount == 1)
            {
                if (time <= 0f)
        ...
        public static bool MaxAmountCheck
        {
            get
            {
                return inst.advancedDemolitionProperties.currentAmount < inst.advancedDemolitionProperties.maximumAmount;
            }
        }
    }
}
