...
namespace RayFire
{
    ...
    public class RayfireGun : MonoBehaviour
    {
        ...
        public void Shoot(Vector3 shootPosition, Vector3 shootVector)
        {
            ...
            Transform tm = hit.transform;

            // Check for tag
            if (tagFilter != "Untagged" && tag != tm.tag)
                return;
            ...
            RayfireRigid scrRigid = tm.GetComponent<RayfireRigid>();
            ...
            if (scrRigid.damage.enable == true)
            {
                ...
                if (damageDemolition == true && scrRigid.fragments != null && scrRigid.fragments.Count > 0)
                {
        ...
        List<RayfireRigid> ActivationCheck(RayfireRigid scrTarget, Vector3 position, float radius)
        {
            ...
            List<RayfireRigid> rigidList = new List<RayfireRigid> { scrTarget };
